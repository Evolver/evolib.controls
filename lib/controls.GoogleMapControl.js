...
// phrase namespace
var phraseNamespace ='GoogleMapControl';
...
var GoogleMapControl =api.GoogleMapControl =function( object) {
		...
		{// create tools
			...
			{// create 'search address' tool
				...
				{// create title
					...
					lib.html.set( title, GetPhrase( 'findAddress'));
	...
	if( lib.attr.has( object, 'data-map-zoom')) {
		...
		this.mapZoom =parseInt( lib.attr.get( object, 'data-map-zoom'));
	}
	...
	lib.event.bind( this.object, 'click', this.__objectClickHandler =function( e){
...
// (read-only) map's default zoom level
p.mapZoom =8;
...
p.__findAddress =function( address) {
	...
	if( address =='')
		...
		return alert( GetPhrase( 'error.specifyAddress'));
	...
	this.geocoder.geocode( req, function( results, status) {
		...
		if( results.length ==0) {
			// no results found
			return alert( GetPhrase( 'error.addressNotFound', {'address': address}));
		}
...
p.__handleMapClick =function( latLng) {
	...
	// create new point
	var newPoint =new GoogleMapPoint( this, latLng.lat(), latLng.lng());
	
	// add new point to the map (FIXME: only for multi maps. For single maps - reposition existing point)
	this.selectPoint( newPoint);
	
	// open point's info window
	this.__showInfoWindow( newPoint.infoWindow, newPoint.marker);
};
